...
describe('Windows visual-smoke fixed backdrop contract', () => {
  ...
  it('proves first-appearance hit testing and half-head hover reveal on the same HWND', () => {
    ...
    expect(hoverGate).toContain('Get-AvatarClickPoint -MascotWindow $revealedMascot')
    expect(hoverGate).toContain('$settledAvatarPoint.X')
    expect(hoverGate).toContain('$settledAvatarPoint.Y')
