...
describe('Windows administrator deployment smoke contracts', () => {
  it('requires the machine-wide desktop authentication callback protocol throughout MSI lifecycle gates', () => {
    ...
    expect(deployment).toContain("[string]$nativeDiagnostic.fields.callbackUrl -notin @('', '[redacted]')")
    expect(deployment).toContain('$nativeDiagnostic.fields.callbackUrlLength -eq ([string]$nativeDiagnostic.fields.callbackUrl).Length')
    expect(deployment).toContain("[string]$rendererDiagnostic.fields.rawUrl -notin @('', '[redacted]')")
    expect(deployment).toContain('$rendererDiagnostic.fields.rawUrlLength -eq ([string]$rendererDiagnostic.fields.rawUrl).Length')
    expect(deployment).toContain("$_.fields.outcome -eq 'success'")
