  ...
  if ($ValidateDefaultLaunch) {
    ...
    if ($currentSessionId -in $interactiveSessions) {
      ...
      $report.checks.defaultLaunch = [ordered]@{
        ...
        manualCommand = 'powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\Test-HualiAISilentDeployment.ps1 -MsiPath .\<1.0.40.msi> -ExpectedVersion 1.0.40 -ValidateDefaultLaunch -RequireInteractiveDefaultLaunch'
      }
